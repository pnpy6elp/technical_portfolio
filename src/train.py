...
def evaluate(args, dataset, data_stream, memory_banks, flush=True):
    ...
    for i in range(args.repeat):
        ...
        for mmm in memory_bank:
            if 'edge__igraph_index' in mmm.keys():
                del mmm['edge__igraph_index']
        # print(memory_bank)
    ...
    print(f"mAP: {np.mean(mAPs):.2f}±{np.std(mAPs, ddof=1):.2f}", flush=flush)
...
def main():
    ...
    parser.add_argument('--cls-epoch', type=int, default=500)
    ...
    parser.add_argument('--minor_thres', type=int, default=100)
    parser.add_argument('--community_algo', type=str, default='multilevel')
    parser.add_argument('--max_supernodes', type=int, default=20)
    parser.add_argument('--top_k', type=int, default=20)
    # 
    parser.add_argument('--fixed_supernode_count', type=int, default=20)

    parser.add_argument('--community_weight', type=str, default=0.3)
    # minority_ratio
    parser.add_argument('--minority_ratio', type=str, default=0.5)
    ...
    parser.add_argument('--feature_method', type=str, default='contrastive')
    

    args = parser.parse_args()
    print(args)
    args.minority_ratio = float(args.minority_ratio)
    args.community_weight = float(args.community_weight)
    ...
    if os.path.exists(task_file):
        ...
        if args.dataset_name == 'corafull_mixed':
            data_stream.tasks = data_stream.tasks[:35]
        elif args.dataset_name in ['reddit_mixed','arxiv_mixed']:
            data_stream.tasks = data_stream.tasks[:20]
        elif args.dataset_name in ['products_mixed']:
            data_stream.tasks = data_stream.tasks[:23]
    ...
    if args.cgl_method in ["bare", "ewc", "mas", "gem", "twp", "lwf"]:
        ...
        print(f"Time: {np.mean(tmm):.1f}±{np.std(tmm, ddof=1):.1f}", flush=True)

        tt2 = time.time() - tt1
        print(f"Overall runtime: {tt2} secs")
